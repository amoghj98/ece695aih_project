...
def plot_custom(x_col, y_col, xlabel, ylabel, title, filename, logx=False, logy=False, log_base=10):
    ...
    for (model, algo, hw), group in grouped:
        ...
        for i, (_, row) in enumerate(group_sorted.iterrows()):
            ax.plot(row[x_col], row[y_col], marker=marker, color=color,
                     linestyle='None', label=label if i == 0 else None)

        ax.plot(group_sorted[x_col], group_sorted[y_col], linestyle=linestyle,
    ...
    # Consolidated Legend block with headers
    legend_elements = []
    legend_labels = []
    ...
    # Section: Model, Algo, Hardware combinations
