...
@dataclass
class Config:
    approach: Literal["best_of_n", "beam_search", "dvts", "vanilla"] = "best_of_n"
    ...
    #prm_path: str = "Qwen/Qwen2.5-Math-PRM-7B"
    ...
    top_p: float = 1.0
    prm_batch_size: int = 1 # reducing from 4 to 1
    search_batch_size: int = 25
    seed: int = 42
    max_tokens: int = 32756
    ...
    def __post_init__(self):
        ...
        if self.push_to_hub:
            ...
            elif self.approach == "vanilla":
                self.revision = f"{self.dataset_name.replace('/', '_')}--T-{self.temperature}--seed-{self.seed}"
            else:
